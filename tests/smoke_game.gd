...
func run_smoke() -> void:
	...
	check(float(game.CAMP_STRUCTURE_LAYOUT.veterans_hall.anchor.y) == 360.0 and float(game.CAMP_STRUCTURE_LAYOUT.campfire.anchor.y) == 678.0, "visible structure bases remain centered on the painted foundation stones")
