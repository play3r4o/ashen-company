...
## Directional compact palisade kit

Generated with the built-in image generation tool in reference-guided generate mode, using the existing straight wall, old corner and Veterans' Hall as style references. The original chroma atlas is retained at `assets/foundation/sources/palisade_directional_atlas_v1_chroma.png`; the alpha-processed atlas is `assets/foundation/sources/palisade_directional_atlas_v1_alpha.png`. Runtime pieces are under `assets/foundation/town/palisade`.
