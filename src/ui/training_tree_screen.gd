...
const NodeCardScene = preload("res://src/ui/training_node_card.tscn")
const ConnectorScene = preload("res://src/ui/training_connector.tscn")
...
@onready var tier_label: Label = $Header/TierLabel
...
func _build_graph() -> void:
	if not is_instance_valid(canvas):
		canvas = Control.new()
		canvas.name = "TrainingTreeCanvas"
		canvas.position = Vector2.ZERO
		canvas.custom_minimum_size = Vector2(2200.0, 2200.0)
...
func _node_size(node_type: String) -> Vector2:
	match node_type:
		"root", "utility": return Vector2(150.0, 48.0)
		"weapon": return Vector2(142.0, 58.0)
		"technique": return Vector2(142.0, 58.0)
		"doctrine": return Vector2(148.0, 58.0)
		"keystone": return Vector2(148.0, 58.0)
		_: return Vector2(126.0, 44.0)

func _graph_point(definition_position: Vector2) -> Vector2:
	return definition_position + Vector2(1100.0, 1100.0)

func _node_style(node_id: String, hover: bool) -> StyleBoxFlat:
	var definition: Dictionary = Content.all_nodes()[node_id]
	var purchased: bool = service.node_rank(node_id) > 0
	var available: bool = _node_available(node_id)
	var style := StyleBoxFlat.new()
	var base_color: Color = Color("303537")
	var border_color: Color = Color("555b5d")
	if purchased:
		base_color = Color("3e5d50")
		border_color = Color("d6aa57")
	elif available:
		base_color = Color("454b4e")
		border_color = Color("ba9e63")
	else:
		base_color = Color("202426")
		border_color = Color("555b5d")
	if node_id == selected_node_id:
		border_color = Color("fff0bd")
		base_color = base_color.lightened(0.08)
	if hover:
		border_color = Color("eee0bc")
...
func _update_header() -> void:
	...
	tier_label.text = "TRAINING GROUNDS TIER %d" % int(profile.get("training_level", 0))
...
func _center_school(school: String) -> void:
	...
	if best.is_empty():
		return
	zoom = 0.72
	var target: Vector2 = _graph_point(Vector2(Content.all_nodes()[best].position))
