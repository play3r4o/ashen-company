...
|---|---|---|---|---|---|
| Application composition | one-node `main.tscn` | `scenes/app/game_root.tscn` | `GameCoordinator` and typed controllers | IN PROGRESS | split remaining 5k-line controller; coordinator <=800 lines |
...
| Training Grounds | `src/ui/training_tree_screen.*` | `scenes/ui/screens/training_tree_screen.tscn` | screen + Training service | MIGRATED | move orchestration out of `main.gd` |
| Expedition Arsenal | `src/ui/arsenal_screen.*` | `scenes/ui/screens/arsenal_screen.tscn` | screen + Arsenal service | MIGRATED | move orchestration out of `main.gd` |
| Results | rectangle library/code binding | `scenes/ui/screens/results_screen.tscn` | `AshenResultsScreen` | MIGRATED | none |
| Level-up | code-built cards | `scenes/ui/overlays/level_up_overlay.tscn` + card scene | overlay + offer service | MIGRATED | remove one dead legacy binder |
| Relic/contract choices | code-built panels | authored overlay scenes | overlay scripts | MIGRATED | move orchestration out of `main.gd` |
| Gate/reset/dismantle confirmations | code-built panels | authored confirmation overlay scenes | overlay scripts | MIGRATED | mount through `UIController.ModalLayer` |
| Hall/construction/building/class/offline | visual guides and builders | complete scenes under `scenes/ui/screens/` | `AshenCampListScreen` | MIGRATED | move flow orchestration out of `main.gd` |
| Inventory | code-built inventory panel | `scenes/ui/screens/inventory_screen.tscn` | inventory screen binding | IN PROGRESS | split reusable item-card scene and controller ownership |
| Camp tiers 0-4 | `src/foundation/camp_layout*` + camp draw layer | `scenes/world/camp/camp_tier_*.tscn` | `AshenCampRuntime` | MIGRATED | controller extraction only |
...
| Structures and props | draw dictionaries and texture bounds | reusable scenes under `scenes/world/structures` and `props` | scene scripts | MIGRATED | complete dedicated-art review remains |
| Actors | actor draw path | scenes under `scenes/actors/` | actor presentation controller | MIGRATED | move gameplay ownership to player/enemy controllers |
| Projectiles | circles/lines in the main draw path | scenes under `scenes/combat/projectiles/` | combat presentation controller | MIGRATED | crossbow/legacy shared-art review |
| Pickups/effects/hazards | production draw primitives | scenes under `scenes/combat/` | combat presentation controller | IN PROGRESS | BLOCKED: ART for dedicated pickup/effect textures |
| Procedural terrain | retained `_draw()` chunks | TileSet + TileMapLayer in `scenes/world/terrain/` | terrain scene + region service | MIGRATED | add remaining barrier/landmark scene coverage |
