...
|---|---|---|---|---|---|
| Application composition | one-node `main.tscn` | `scenes/app/game_root.tscn` | `GameCoordinator` and typed controllers | MIGRATED | root coordinator is 61 lines; compatibility `src/main.gd` is 4 lines |
...
| Hall/construction/building/class/offline | visual guides and builders | complete scenes under `scenes/ui/screens/` | `AshenCampListScreen` | MIGRATED | none |
...
| Projectiles | circles/lines in the main draw path | scenes under `scenes/combat/projectiles/` | combat presentation controller | MIGRATED | none |
| Pickups/effects/hazards | production draw primitives | scenes under `scenes/combat/` | combat presentation controller | MIGRATED | dedicated canonical combat atlas is active |
